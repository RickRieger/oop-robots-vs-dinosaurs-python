...
class Dinosaur:
  ...
  def attack(self, robot, fleet_of_robots):
    ...
    if self.energy <= 0:
      ...
      print(f'\n\nBAM! DESTRUCTION! =====> The {self.attack_name} that {self.name} gave {robot.name} proved to be just enough to take {robot.name} out of service. Say bye-bye to {robot.name}')
